<div @class([$this->getClass('wrapper')])>
    ...
    <div @class([$this->getClass('table_wrapper')])>
        <table @class([$this->getClass('table')])>
                    ...
                    @foreach ($columns as $key => $column)
                        <th scope="col" @class([$this->getClass('th')])>
                            @if (in_array($key, $sortable))
                                <button wire:click="sortBy('{{ $key }}')" wire:key="sort-{{ $key }}"
                                    @class([$this->getClass('th_sort_button')])>
                                    <span>{{ $column }}</span>
                                    <span @class([$this->getClass('th_sort_icon_wrapper')])>
                                        @if ($sortField === $key)
                                            @if ($sortDirection === 'asc')
                                                <svg @class([$this->getClass('th_sort_icon_active')]) xmlns="http://www.w3.org/2000/svg"
            ...
            <tbody @class([$this->getClass('tbody')])>
